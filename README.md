...
Latest progress timestamp: `2026-05-20T20:27:33-07:00`.
...
| `2026-05-20T20:27:33-07:00` PGN header timestamp | Root tactical prior probe (`reports/2026-05-20_tactical_prior_probe.md`, `reports/policyhead192_stability198_tacticalprior035_shallowguards_stockfish_gate.pgn`). | N/A | shallow-guard tactical-prior gate `0.0/2` | Added an opt-in cheap root tactical prior blend for MCTS. The first full-depth variant was too slow to complete, and a practical shallower-guard setting changed the games but still lost both. |
...
- `--good-action-book-top-k` now applies to combined action/policy teacher
...
- Stronger bad-action margin pressure on that same 72-position source either
  leaves the target ranks mostly unchanged or damages broad holdout before the
  targets become plausible. This is not just a margin-weight tuning problem.
...
- Fresh exact context coverage can remove the immediately repeated `Bc4` and
  ...
  path.
- The king-safety guard now searches quiet pressure moves as well as tactical
  moves, catching a local continuation from the newest White failure. In full
  direct play, the losses shifted again and the score remained `0.0/2`.
...
- Root tactical priors can now softly reweight MCTS root choices with cheap
  material/king-safety scores after exact books and hard guards run. The first
  practical setting still scored `0.0/2`, so this is search-calibration
  infrastructure rather than a strength breakthrough.
- The material guard no longer forces one speculative checking-capture
  ...
  tactically.
- A focused replay on the sac-guard losses improves that tiny slice's
...
- Material and king-safety guards now compute independent safe sets before
  fallback, preventing either guard from starving the other on the latest
  `...Nxf2` style sacrifice. The direct gate still fails after the bad root
  choices shift.
...
- When both material and king-safety have no threshold-safe root move, the
  guard stack now prefers the material fallback unless that fallback is only
  king moves. This removed the follow-up `...Nxf2` sacrifice from the latest
  direct gate, but the score stayed `0.0/2`.
- A lower-pressure replay on the newest material-fallback losses preserved the
  broad holdout and improved the new slice's bad-action loss slightly, but the
  direct gate again stayed `0.0/2`. The failures shifted to new `Rxb2`,
  passed-pawn, and king-attack lines.
- A broader full-game replay over the latest 80 Stockfish gate PGNs gives a
...
- Raising the same broader replay checkpoint from 16 to 64 simulations also
  ...
  at the direct gate.
- A low-LR full-network pass on the full-game slice improved broad holdout and
  full-game target metrics more than policy-head-only tuning, but the direct
  gate still stayed `0.0/2`.
...
- Blending a rejected checkpoint back toward the parent is promising as a cheap
  ...
  holdout guard, but its recreated direct gate still lost both Stockfish games.
  The broad top-k guard is useful but still not sufficient for direct strength.
...
- The latest root-filter refinements remove specific bad move families rather
  than improving measured strength; all comparable follow-up gates have stayed
  `0.0/2`.
