...
LEAF_MATERIAL_VALUE_WEIGHT="${LEAF_MATERIAL_VALUE_WEIGHT:-0.0}"
LEAF_MATERIAL_SEARCH_PLIES="${LEAF_MATERIAL_SEARCH_PLIES:-0}"
...
SELECT_BEST_BY_ARG=""
if [[ -n "$SELECT_BEST_BY" ]]; then
  SELECT_BEST_BY_ARG="--select-best-by $SELECT_BEST_BY"
fi
REPLAY_DATA_ARG=""
...
gpu-dev submit \
  ...
  -- bash -lc "
    ...
    uv run alpha-chess iterate \
      ...
      --leaf-material-value-weight $LEAF_MATERIAL_VALUE_WEIGHT \
      --leaf-material-search-plies $LEAF_MATERIAL_SEARCH_PLIES \
      ...
      --root-king-safety-search-plies $ROOT_KING_SAFETY_SEARCH_PLIES \
      --root-king-safety-max-loss-cp $ROOT_KING_SAFETY_MAX_LOSS_CP \
